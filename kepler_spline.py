...
def kepler_spline(time, flux, bkspace=1.5, maxiter=5, outlier_cut=3):
    ...
    for _ in range(maxiter):
        if spline is None:
            ...
            _, _, new_mask = robust_mean(residuals, cut=outlier_cut)
        ...
        try:
            with warnings.catch_warnings():
                ...
                curve = bspline.iterfit(
                    time[mask], flux[mask], bkspace=bkspace)[0]
...
def choose_kepler_spline(all_time,
    ...
    for bkspace in bkspaces:
        ...
        for time, flux in zip(all_time, all_flux):
            ...
            # From the bspline implementation.
        ...
        likelihood_term = npoints * \
            np.log(2 * np.pi * sigma**2) + ssr / sigma**2
