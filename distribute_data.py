...
def distribute_data(tce_list, class_list, data_type):
    ...
    for i in range(list_length):
        ...
        kepid_dir = os.path.join(
            environment.KEPLER_DATA_FOLDER, kep_id[0:4], kep_id)
        ...
        dest_folder = os.path.join(
            environment.TRAINING_FOLDER, data_type, label_folder)
        if not os.path.exists(dest_folder):
            os.mkdir(dest_folder)

        print("Copying {0} to {1}, {2:5d} of {3}".format(
            file_name, dest_folder, i, list_length))
...
def main():
    ...
    tce_table = pd.read_csv(environment.KEPLER_CSV_FILE,
                            comment="#")
    ...
    allowed_tces = tce_table[environment.LABEL_COLUMN].apply(
        lambda l: l in environment.ALLOWED_LABELS)
    tce_table = tce_table[allowed_tces]
    num_tces = len(tce_table)
    # tf.logging.info("Filtered to %d TCEs with labels in %s.", num_tces,
    #                 list(_ALLOWED_LABELS))
    print("Filtered to {} TCEs with labels in {}.".format(
        num_tces, list(environment.ALLOWED_LABELS)))
