...
def download_one_kepler_id_files(kepid):
    ...
    strfolder = os.path.join(
        environment.DATA_FOR_PREDICTION_FOLDER, kepid_formatted[0:4])

    if not os.path.exists(strfolder):
        os.mkdir(strfolder)

    strfolder = os.path.join(
        environment.DATA_FOR_PREDICTION_FOLDER, kepid_formatted[0:4], kepid_formatted)
    ...
    try:
        ...
        files = [node.get('href') for node in soup.find_all(
            'a') if node.get('href').endswith('_llc.fits')]
        # print(files)
